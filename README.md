...
Components Used:
Linked Services → Connect to sources (GitHub, SQL)
Datasets → Define data structure
Pipelines → Orchestrate workflows
Copy Activity → Move data
Flow:
Source → ADF Pipeline → ADLS Gen2 (Raw Layer)
Key Features:
No-code / low-code
...
Flow:
Databricks → Fetch data from MongoDB → Join → Enriched dataset
Why Use MongoDB?
...
Flow:
ADLS (Processed Data) → Synapse → BI Tools
Why Synapse?
